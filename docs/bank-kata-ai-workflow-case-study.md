...
The study does not name a universal framework. It records which framework or workflow matched each goal: compact specification review, design control before implementation, detailed implementation steps, or few user interruptions.
...
## Future auditability of generated files
...
This changes one interpretation: Spec Loop was not unique in documenting rationale, but it preserved implementation reasoning inside task files alongside research, glossary/terms, constraints, design, and test specifications. OpenSpec was the closest compact alternative for decision rationale.
...
| `spec-loop-base-backlog-steered` | Recorded concrete decisions with reasons and confidence levels; later repaired missing rollback and storage-write-failure decisions after user challenge. | User challenge improved rollback and storage-write-failure handling. |
...
| `superpowers` | Asked product/design questions and recorded user answers for localStorage, Daily/Savings, stack, transfer visibility, date filtering, and test scope. | Source and tests were still needed to check whether those answers reached the final code. |
...
| `superpowers-calisthenics` | Corrected date-control design after user challenge. | The final code/tests still preserved fewer constraints than requested. |
...
| Solution | Tests | Behavior evidence (full / partial / missing) | Design score (0–18) | Code/test conclusion |
|---|---:|---:|---:|---|
| `spec-loop-base-backlog-steered` | 60/60 | 13 full / 1 partial / 1 missing | 17/18 | Highest design score in this comparison. Tests covered broad behavior, localStorage validation, and rollback depth. User intervention improved this solution. |
| `spec-loop-base-backlog-prompted` | 58/58 | 13 / 1 / 1 | 14/18 | 58 passing tests and 13 full behavior categories. Restored localStorage validation and storage-write-failure handling during deposit/withdrawal were less complete than in `spec-loop-base-backlog-steered`. |
| `spec-loop-incremental` | 30/30 | 13 / 1 / 1 | 13/18 | 30 passing tests and 13 full behavior categories. Planning was kept in one task file rather than separate backlog files. |
...
| `open-spec` | 19/19 | 11 / 0 / 1 | 12/18 | Base-scope implementation with 19 passing tests and 11 full behavior categories. Few design decisions were visible in session. |
| `superpowers` | 17/17 | 11 / 1 / 3 | 12/18 | Accepted product decisions were visible in session. Tests did not cover UI flow, localStorage validation, print assertions, or storage-write failure as fully as the top behavior-evidence solutions. |
...
| `superpowers-calisthenics` | 13/13 | 8 / 3 / 4 | 11/18 | Session included correction of date-control design. Final tests/code preserved fewer requested constraints and required behaviors than the top behavior-evidence solutions. |
| `superpowers-5.4` | 16/16 | 7 / 5 / 3 | 11/18 | 16 passing tests, but only 7 full behavior categories. |
...
Static metrics did not choose the winner. They explained specific risks. The facts below come from the metric sweep of the original eleven solutions; `gsd-small-feature` was added later and was not included in those generated metric result files.
...
Interpretation: static metrics identified specific risks, but behavior evidence and design locality explained the important differences better.

## Patterns behind the top results
...
Superpowers produced detailed plans and asked many product questions. The final solutions still had fewer full behavior-evidence categories than the Spec Loop backlog solutions. GSD Small Feature in GSD Pi produced a shorter plan and a working app, but it also had fewer committed tests and less committed rationale.
...
This ranking gives more weight to behavior evidence, implementation safety, written or discussed decisions before code, and source/test design score than to compact files or conversation volume alone.

A source-design-first ranking differs slightly: `open-spec-calisthenics` rises because its domain language and boundaries scored high. That does not override the broader ranking because behavior evidence and storage-write-failure tests still matter.
