...
increment rather than clarifying or reviewing the current increment,
do not continue in implementation mode by inertia. Return that new
work item to `spec-loop-plan-task` for route selection before more
executable work continues.
